# BBR v3 优化脚本 - Ultimate Edition v5.4.3

**XanMod 内核 + BBR v3 + 全方位 VPS 管理工具集**

一键安装 XanMod 内核，启用 BBR v3 拥塞控制，集成 33 项实用功能，优化你的 VPS 服务器。

> **版本**: v5.4.3 🔧 **修复**：菜单33「端口流量计费与到期管理」补齐开机自动恢复（重启后自动重建计数/配额/限速并重新封锁到期端口）、配额80%/95%阈值邮件通知、到期前3天预警；修正 tc 限速卸载会误删同网卡其他端口规则、配额规则重复插入等问题；修复 Sub2API（菜单32）自定义端口部署后显示恒为默认 8282、修改端口静默失效的问题（端口现以 systemd 服务文件为准）。
